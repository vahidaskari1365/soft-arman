    ...
    <div>
      ...
      <div className="print-area mb-4 break-after-page border-2 border-slate-800 bg-white p-3 text-slate-900" style={{ width: "200mm" }}>
        ...
        <div className="flex items-center justify-between border-b-2 border-slate-800 pb-1.5">
          <div className="flex items-center gap-2">
            {cfg.logo ? (
              ...
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-slate-800 text-white text-sm font-bold">H</div>
            )}
            <div>
          ...
          <div className="text-center">
            <p className="rounded bg-slate-800 px-2 py-0.5 text-[10px] font-bold text-white">رسید پذیرش دستگاه</p>
            <p className="mt-0.5 text-[10px] font-bold text-slate-500">{copyLabel}</p>
            <div className="mt-0.5 flex flex-col items-end gap-0.5 text-[9px] text-slate-600">
        ...
        <div className="mt-2 rounded-lg border border-slate-300 p-2">
          <p className="mb-1 border-b border-slate-200 pb-1 text-[10px] font-extrabold text-slate-700">اطلاعات مشتری و دستگاه</p>
        ...
        {d.customerAddress && (
          <div className="mt-2 rounded-lg border border-slate-300 px-2 py-1 text-[10px]"><span className="text-slate-500">آدرس: </span>{d.customerAddress}</div>
        )}
        ...
        <div className="mt-2 flex items-stretch gap-3">
          <div className="w-2/5 overflow-hidden rounded-lg border border-slate-300">
            <p className="border-b border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-extrabold text-slate-700">خلاصه مالی</p>
            <table className="w-full text-[10px]"><tbody>
              <tr><td className="border-b border-slate-100 px-2 py-1">هزینه تخمینی تعمیر</td><td className="border-b border-slate-100 px-2 py-1 text-left font-mono">{formatMoney(d.estimatedCost || 0)}</td></tr>
              <tr><td className="border-b border-slate-100 px-2 py-1">پیش‌پرداخت / بیعانه</td><td className="border-b border-slate-100 px-2 py-1 text-left font-mono">{formatMoney(d.deposit || 0)}</td></tr>
              <tr className="font-extrabold"><td className="px-2 py-1">مبلغ نهایی قابل پرداخت</td><td className="px-2 py-1 text-left font-mono">{formatMoney(d.finalCost || d.estimatedCost || 0)}</td></tr>
            </tbody></table>
          </div>

          <div className="flex-1 rounded-lg border border-slate-300 p-2">
            <div className="flex items-end justify-between gap-4 text-[10px] text-slate-600">
              <div className="w-1/2 text-center">
                <div className="border-t border-slate-500 pt-1.5">امضا مشتری</div>
                <div className="mt-1 font-semibold text-slate-900">{customerName}</div>
              </div>
              <div className="w-1/2 text-center">
                <div className="border-t border-slate-500 pt-1.5">امضا کارشناس پذیرش</div>
                <div className="mt-1 font-semibold text-slate-900">{intakeName}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between border-t border-slate-200 pt-1.5">
          <div className="flex items-center">
            <TicketBarcode value={d.ticketNumber} height={28} />
          </div>
          <div className="max-w-sm text-[9px] leading-tight text-slate-500">
